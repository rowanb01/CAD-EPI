...
# 305524      2
dim(CombinedLiftOver[match(intersect(unique(CombinedLiftOver$Original_ID), CombinedLiftOver$Original_ID), CombinedLiftOver$Original_ID),])
# 152762      2
...
# 321392      7
dim(Auto_LiftOver_eMERGE_PCs[match(intersect(Auto_LiftOver_eMERGE_PCs$Original_ID, Auto_LiftOver_eMERGE_PCs$Original_ID), Auto_LiftOver_eMERGE_PCs$Original_ID),])
# 321334     12
dim(Auto_LiftOver_eMERGE_PCs[match(intersect(Auto_LiftOver_eMERGE_PCs$New_ID, Auto_LiftOver_eMERGE_PCs$New_ID), Auto_LiftOver_eMERGE_PCs$New_ID),])
# 321334     12
#A key point here is that only half of the snps make it through the intersection phase, which is important to state
...
# 152762      7
#Point is proven again that this number that are found in the intersect of the variants and the imputed file is around 150K
TrueLifOver_Intersect_InfoFile <- CombinedLiftOver[match(intersect(unique(CombinedLiftOver$Original_ID), CombinedLiftOver$Original_ID), CombinedLiftOver$Original_ID),]
dim(TrueLifOver_Intersect_InfoFile)
# 152762      2
...
# 152762      4
head(TrueLifOver_Intersect_InfoFile$Original_ID, 5)
...
Reset_PC_Loadings_eMERGE$SNP <- TrueLifOver_Intersect_InfoFile$New_ID
Reset_MeanSD_eMERGE$SNP <- TrueLifOver_Intersect_InfoFile$New_ID
Newsites <- TrueLifOver_Intersect_InfoFile$New_ID
write.table(Reset_PC_Loadings_eMERGE, "Norsid_hg38_intersects_BioMe_eMERGE_prs_adjustment.pc.loadings", col.names=T, row.names=F, quote=F, sep="\t")
write.table(Reset_MeanSD_eMERGE, "Norsid_hg38_intersects_BioMe_eMERGE_prs_adjustment.pc.meansd", col.names=T, row.names=F, quote=F, sep="\t")
write.table(Newsites, "Norsid_hg38_intersects_BioMe_eMERGE_prs_adjustment.pc.sites", col.names=F, row.names=F, quote=F, sep="\t")
write.table(TrueLifOver_Intersect_InfoFile, "Masterfile_for_FlashPCA_intersects_Info_File_062725.txt", col.names=T, row.names=F, quote=F, sep="\t")
q()
